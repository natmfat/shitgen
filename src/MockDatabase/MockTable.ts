...
export class MockTable extends MockEntity {
   ...
   */
  generateModelData(database: MockDatabase) {
    const modelData: string[] = [`export type ${this.formattedName}Data = {`];
    for (const column of this.columns) {
      modelData.push(`  ${column.name}: ${column.generateType(database)};`);
    }
    modelData.push("}");
  ...
  toString(database: MockDatabase) {
    return [
      this.generateModelData(database),
      this.generateModelAutoGenerated(),
      this.generateModelOptional(),
      this.generateModelRelationship(database),
      this.generateModel(),
    ].join("\n");
  }
}
